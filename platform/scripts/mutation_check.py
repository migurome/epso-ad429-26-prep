...
MUTATIONS = [
    ...
    (
        "lo de Drive sustituye al progreso local en vez de fusionarse",
        "src/lib/driveSync.ts",
    ...
    (
        "pegar el identificador de Google no cambia el estado de la tarjeta",
        "src/lib/driveStore.ts",
        "        set({ clientId: clientId.trim(), status: clientId.trim() ? 'disconnected' : 'off', error: null }),",
        "        set({ clientId: clientId.trim(), status: 'off', error: null }),",
        "src/components/DriveSyncCard.test.tsx",
    ),
]
