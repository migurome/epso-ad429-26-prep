...
Spanish translations) and generates platform/src/data/content.generated.ts
(QUESTIONS, THEORY_DOCS, ESSAY_PROMPTS). Questions are bilingual: prompt,
...
DOCS = ROOT / "Docs"
OUT = Path(__file__).resolve().parent.parent / "src" / "data" / "content.generated.ts"
...
def build_field_mcq():
    ...
    for cfg in FIELD_MCQ_CONFIGS:
        try:
            ...
            raise ValueError(f"build_field_mcq[{cfg['field']}]: {e}") from e
        all_questions += questions
        all_theory.append(theory)
    return all_questions, all_theory
...
def emit_ts(all_questions, all_theory, all_essays):
    lines = []
    ...
    lines.append("")
    lines.append(f"export const QUESTIONS: Question[] = {json.dumps(all_questions, ensure_ascii=False, indent=2)}")
    lines.append("")
    lines.append(f"export const THEORY_DOCS: TheoryDoc[] = {json.dumps(all_theory, ensure_ascii=False, indent=2)}")
    lines.append("")
    lines.append(f"export const ESSAY_PROMPTS: EssayPrompt[] = {json.dumps(all_essays, ensure_ascii=False, indent=2)}")
    lines.append("")
...
def main():
    ...
    for build_fn in (build_verbal, build_numerical, build_abstract, build_field_mcq):
        try:
    ...
    print(f"\nTOTAL: {len(all_questions)} questions, {len(all_theory)} theory docs, {len(all_essays)} essay prompts")

    emit_ts(all_questions, all_theory, all_essays)
    print(f"Wrote {OUT}")
