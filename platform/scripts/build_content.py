...
BOARD_FILE = DOCS / "board" / "notices.json"
BOARD_FORMAT = 2
BOARD_STAGES = ('open', 'in-progress', 'closed')


def emit_board():
    """Writes src/data/board.generated.ts from what the board bot has seen.

    The bot (scripts/board_fetch.mjs) reads EPSO's listing and keeps
    Docs/board/notices.json; this only reshapes it for the app. Missing file
    is not an error — the bot may not have run yet — but a file we cannot
    trust IS one: a malformed notice would reach the noticeboard as a card
    with holes in it, and the candidate would not know whether EPSO published
    it that way or the platform broke.
    """
    if not BOARD_FILE.exists():
        board = {'format': BOARD_FORMAT, 'sources': [], 'year': None, 'since': None, 'notices': []}
    else:
        board = json.loads(BOARD_FILE.read_text(encoding='utf-8'))
        if board.get('format') != BOARD_FORMAT:
            raise SystemExit(
                f"{BOARD_FILE} está en formato {board.get('format')} y esperábamos el {BOARD_FORMAT}"
            )
        seen = set()
        for notice in board['notices']:
            missing = [k for k in ('id', 'title', 'url', 'firstSeen', 'stage') if not notice.get(k)]
            if missing:
                raise SystemExit(f"convocatoria sin {', '.join(missing)} en {BOARD_FILE}: {notice}")
            if notice['stage'] not in BOARD_STAGES:
                raise SystemExit(f"convocatoria en fase desconocida '{notice['stage']}': {notice['id']}")
            if notice['id'] in seen:
                raise SystemExit(f"dos convocatorias con el mismo id '{notice['id']}' en {BOARD_FILE}")
            seen.add(notice['id'])

    lines = [
        "// AUTO-GENERATED by scripts/build_content.py — do not edit by hand.",
        "// El bot escribe Docs/board/notices.json (node scripts/board_fetch.mjs);",
        "// esto sólo le da forma para la aplicación.",
        "",
        "import type { BoardFile } from '../types/board'",
        "",
        f"export const BOARD: BoardFile = {json.dumps(board, ensure_ascii=False, indent=2)}",
        "",
    ]
    (GEN_DIR / 'board.generated.ts').write_text('\n'.join(lines), encoding='utf-8')
...
def main():
    ...
    notices = emit_board()
    stages = {stage: sum(1 for n in notices if n['stage'] == stage) for stage in BOARD_STAGES}
    print(f"emit_board: {len(notices)} notice(s) — " + ", ".join(f"{v} {k}" for k, v in stages.items()))
